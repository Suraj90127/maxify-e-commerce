...
const Shops = () => {
  ...
  const queryCategoey = (e, value) => {
    if (e.target.checked) {
      setCategory(value);
      const selectedCategory = categorys.find((cat) => cat.name === value);
      setSubCategoryData(selectedCategory ? selectedCategory.subCategory : []);
  ...
  }, [dispatch, state.values, category, rating, pageNumber, sortPrice]);
  ...
  return (
    <div>
      <Headers />
      <section className='bg-[url("http://localhost:3000/images/banner/shop.jpg")] xl:h-[500px] md-lg:h-[360px] sm:h-[150px] mt-6 bg-cover bg-no-repeat relative bg-left'>
                      ...
                      {subCategoryData.length > 0 && subCategoryShow === i && (
                        <div>
                          {subCategoryData.map((item, subIndex) => (
                            <div
                              className="flex justify-start items-center gap-2 py-1 ml-10"
                              key={subIndex}
                            >
                              <input
